...
class TestSnoopedRWIMRequest(IntegrationSetup):

    def setUp(self):
        super().setUp()
        self.trace_event = TraceCommand.SNOOP_RWIM
    ...
    def test_exclusive(self):
        """
        Test snooping a RWIM to a line in the Exclusive state.

        Requires the following sequence:
        - Read line (cache miss, NOHIT from other caches)
        - Snoop a RWIM line (cache hit to E in our cache)
        """
        ...
        # Event 5: Snoop a RWIM to the same address
    ...
    def test_modified(self):
        """
        Test snooping a RWIM to a line in the Modified state.

        Requires the following sequence:
        - Write line (cache miss, NOHIT from other caches)
        - Snoop a RWIM line (cache hit to M in our cache)
        """
    ...
    def test_shared(self):
        """
        Test snooping a RWIM to a line in the Shared state.

        Requires the following sequence:
        - Read line (cache miss, HIT from other caches)
        - Snoop a RWIM line (cache hit to S in our cache)
        """
    ...
    def test_invalid(self):
        """
        Test snooping a RWIM to a line in the Invalid state.

        Requires the following sequence:
        - Snoop a RWIM line (cache miss in our cache)
        """
