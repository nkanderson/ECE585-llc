...
class TestCache(unittest.TestCase):
    ...
    def test_decompose_address(self):
        ...
        self.assertEqual(addr_fields, AddressFields(
            tag=0xFFF, # Top 12 bits
            index=0x3FFF, # Next 14 bits
            byte_select=0x3F)) # Bottom 6 bits
    ...
    def test_cache_line_fill(self):
        ...
        for i in range(1, self.cache.associativity):
            new_addr = addr + (i << (self.cache.index_bits + self.cache.byte_select_bits))
        ...
        replacement_addr = addr + (self.cache.associativity << (self.cache.index_bits + self.cache.byte_select_bits))
